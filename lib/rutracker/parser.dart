...
class Parser {
  ...
  List<Torrent> parseQuery(Document document) {
    ...
    for (int i = 0; i < tracks.length; i++) {
      ...
      if (documents.length > 1) {
        var outerHtml = documents[3].outerHtml.toString();
        Torrent torrent = Torrent(
            forum: documents[2].text.trim(),
            theme: documents[3].text.trim(),
            size: documents[5].text.trim(),
            link: documents[3].outerHtml.substring(
                outerHtml.indexOf("viewtopic.php?t=") + 16,
                outerHtml.indexOf(
                    "\"", outerHtml.indexOf("viewtopic.php?t="))));
  ...
  Book parseBook(Document document, String link) {
    ...
    Document documents = parse(docses.substring(
        docses.indexOf("Описание") + 9, docses.indexOf("<!--/post_body-->")));
