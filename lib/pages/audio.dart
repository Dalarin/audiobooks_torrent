...
class _PlaylistState extends State<Playlist> {
  ...
  Future<List<AudioSource>> createPlaylist() async {
    ...
    for (int i = 0; i < folders.length; i++) {
      if (folders[i].path.endsWith(".mp3")) {
        counter++;
        final Tag? tag = await tagger.readTags(path: folders[i].path);
        String tagTitle = tag?.title ?? "Глава $counter";
        var title = cp1251.symbols.contains(cp1251.decodeCp1251(tagTitle[0])!)
            ? cp1251.decodeCp1251(tag?.title)
            : tagTitle;
        playlist.add(
          AudioSource.uri(
            Uri.parse(folders[i].path),
            tag: MediaItem(
              id: '$counter',
              album: widget.book.author,
              title: title ?? widget.book.title,
    ...
    widget.book.listeningInfo["maxIndex"] = counter.toString();
    return playlist;
  }
  ...
  Future<void> _init() async {
    ...
    try {
      int initialIndex = int.parse(widget.book.listeningInfo["index"] ?? '0');
      int initialPosition =
          int.parse(widget.book.listeningInfo["position"] ?? '0');
      await _player.setAudioSource(_playlist,
          initialIndex: initialIndex,
          initialPosition: Duration(seconds: initialPosition));
      _player.setSpeed(double.parse(widget.book.listeningInfo["speed"] ?? 1.0));
    } catch (e, stackTrace) {
  ...
  @override
  void dispose() {
    widget.book.listeningInfo["index"] = _player.currentIndex!.toString();
    widget.book.listeningInfo["position"] =
        _player.position.inSeconds.toString();
    widget.book.listeningInfo["speed"] = _player.speed.toString();
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        IconButton(
          onPressed: () => player.seek(Duration(
              seconds:
